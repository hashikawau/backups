...
int comb_black( struct line *line )
{
      ...
      int key_sum_point;
      key_sum_point = set_sum_point( line->num_black , ini_point , sum_point );

      while( ini_point[line->num_black - 1] <= line->num_white - line->det_white ){
          j ++;
          ini_point[0] ++;
          if( j >= 3000000 ){ return 0; }                                         /**/

          for( i = 0 ; i < line->num_black - 1 ; i ++ ){
              if( ini_point[i] > line->num_white - line->det_white - sum_point[i] ){
                  ini_point[i] = 0;
                  ini_point[i + 1] ++;
                  key_sum_point = 1;
              }
          ...
          if( key_sum_point ){ key_sum_point = set_sum_point( line->num_black , ini_point , sum_point ); }
      }
...
int line_trial( struct line *line , int check_frame[] )
{
      ...
      if( !comb_black( line ) ){ return -10000; }                                   /**/     /*  F10  */

      double count_trial = 0;
