...
int paint( int ini_point[] , struct line *line , int check_frame[] , int crnt_ini_black[] , int crnt_end_black[] )
{
      ...
      for( i = 0 ; i < line->num_black ; i ++ ){
          if( ini_point[i] - 1 >= 0 ){
              if( copy_check_frame[ini_point[i] - 1] == -100 ){ return 0; }
          }
          if( ini_point[i] + line->black[i] < line->num_frame ){
              if( copy_check_frame[ini_point[i] + line->black[i]] == -100 ){ return 0; }
          }
...
/*      if( !comb_black( line ) ){ return -10000; }  */                                 /**/     /*  F09  */
      ...
      while( ini_point[0] <= line->end_black[0] ){
          ...
          check_return2 = 0;
          for( i = 0 ; i < line->num_frame ; i ++ ){
              if( check_frame[i] == 0 || check_frame[i] == count_trial ){ check_return1 = 1; }
              if( check_frame[i] < 0 )                                  { check_return2 ++; }
          }
          if( check_return1 == 0 && check_return2 != line->num_frame ){ return -10000; }
      }
      for( i = 0 ; i < line->num_black ; i ++ ){
          line->ini_black[i] = crnt_ini_black[i];
          line->end_black[i] = crnt_end_black[i];
      }
printf( "%d %d\n" , line->line_no , count_trial );
