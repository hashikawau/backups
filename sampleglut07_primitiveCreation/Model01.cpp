...
inline float linearInterpolation(float leftValue, float rightValue, float middleValue){
	float ret_value = (middleValue - leftValue) / (rightValue - leftValue);
	return isinf(ret_value)?
			-1.0f:
			ret_value;
}
inline float linearInterpolation(float leftValue, float rightValue, float subValue1, float subValue2, float middleValue){
	...
	float temp = subValue1 + subValue2;
	float a = leftValue - temp + rightValue;
	float b = -2 * leftValue + temp;
	float c = leftValue - middleValue;
	...
	//				ret_intp = -1.0f;
					std::cout << "double solution at " << a << ", " << b << ", " << c << std::endl;
					std::cout << "\t" << ret_intp << ", " << solvation << std::endl;
					solvation = -c /b;
					if(!isinf(solvation)){
						ret_intp = solvation;
					}
	...
	for(int i=0; i< numX; ++i){
		for(int j=0; j< numY; ++j){
			for(int k=0; k< numZ ;++k){
				//======================================
//				scalarField[i][j][k] = (i+0)%2 + (j+0)%2 + (k+0)%2;
...
//				scalarField[i][j][k] = fabs(i - t_x);
				...
				//============================
//				scalarField[i][j][k] = sqrt(
//						1.5*(i - t_x)*(i - t_x) + (j - t_y)*(j - t_y) + (k - t_z)*(k - t_z)) * -4.0;
//				scalarField[i][j][k] += sqrt(
//						(i - t_x -lenDiv*2)*(i - t_x -lenDiv*2) + (j - t_y)*(j - t_y) + (k - t_z)*(k - t_z)) *-0.5;
//				scalarField[i][j][k] += sqrt(
//						(i - t_x +lenDiv*2)*(i - t_x +lenDiv*2) + (j - t_y)*(j - t_y) + (k - t_z)*(k - t_z)) *-0.5;
//
//				scalarField[i][j][k] += sqrt(
...
//	}
	...
	for(int i=0; i< width; ++i){
		for(int j=0; j< height; ++j){
			texture[width * i + j] = (int)(i / 8) * 16 + 32;
...
//	x_y_z_index_tetra tetraInfos;
	...
	typedef std::map<Index, TetraHedronInfo> map_index_tetra;
	map_index_tetra tetraInfos;
			...
			for(int k=0; k< numZ ;++k){
				...
				if(i < numX -1){
					...
					if(intpPos >= 0.0f && intpPos <= 1.0f){
						int latticeP = 2;
						const Normal n = linearInterpolation(normalField[i][j][k], normalField[i+1][j][k], intpPos);
//#pragma omp critical
{
						int vertIndex = vertInfos.size();
						vertInfos.push_back(VertexInfo(
								i, j, k,
...
//						tetraInfos[i][j][k][1].addVertexInfo(1, direction, vertIndex);
						tetraInfos[Index(i, j, k, 0)].addVertexInfo(2, direction, vertIndex);
						tetraInfos[Index(i, j, k, 1)].addVertexInfo(1, direction, vertIndex);

						if(j > 0){
//							tetraInfos[i][j-1][k][3].addVertexInfo(5, direction, vertIndex);
							tetraInfos[Index(i, j-1, k, 3)].addVertexInfo(5, direction, vertIndex);
						}
						if(k > 0){
//							tetraInfos[i][j][k-1][2].addVertexInfo(5, -direction, vertIndex);
							tetraInfos[Index(i, j, k-1, 2)].addVertexInfo(5, -direction, vertIndex);
						}
						if(j > 0 && k > 0){
//							tetraInfos[i][j-1][k-1][4].addVertexInfo(3, -direction, vertIndex);
//							tetraInfos[i][j-1][k-1][5].addVertexInfo(4, -direction, vertIndex);
							tetraInfos[Index(i, j-1, k-1, 4)].addVertexInfo(3, -direction, vertIndex);
							tetraInfos[Index(i, j-1, k-1, 5)].addVertexInfo(4, -direction, vertIndex);
						}
}
					}
				}
				...
				if(j < numY -1){
					...
					if(intpPos >= 0.0f && intpPos <= 1.0f){
						int latticeP = 1;
						const Normal n = linearInterpolation(normalField[i][j][k], normalField[i][j+1][k], intpPos);
//#pragma omp critical
{
						int vertIndex = vertInfos.size();
						vertInfos.push_back(VertexInfo(
...
//						tetraInfos[i][j][k][4].addVertexInfo(2, direction, vertIndex);
						tetraInfos[Index(i, j, k, 3)].addVertexInfo(1, direction, vertIndex);
						tetraInfos[Index(i, j, k, 4)].addVertexInfo(2, direction, vertIndex);

						if(k > 0){
//							tetraInfos[i][j][k-1][5].addVertexInfo(5, direction, vertIndex);
							tetraInfos[Index(i, j, k-1, 5)].addVertexInfo(5, direction, vertIndex);
						}
						if(i > 0){
//							tetraInfos[i-1][j][k][0].addVertexInfo(5, -direction, vertIndex);
							tetraInfos[Index(i-1, j, k, 0)].addVertexInfo(5, -direction, vertIndex);
						}
						if(k > 0 && i > 0){
//							tetraInfos[i-1][j][k-1][1].addVertexInfo(4, -direction, vertIndex);
//							tetraInfos[i-1][j][k-1][2].addVertexInfo(3, -direction, vertIndex);
							tetraInfos[Index(i-1, j, k-1, 1)].addVertexInfo(4, -direction, vertIndex);
							tetraInfos[Index(i-1, j, k-1, 2)].addVertexInfo(3, -direction, vertIndex);
						}
}
					}
				}
				...
				if(k < numZ -1){
					...
					if(intpPos >= 0.0f && intpPos <= 1.0f){
						int latticeP = 4;
						const Normal n = linearInterpolation(normalField[i][j][k], normalField[i][j][k+1], intpPos);
//#pragma omp critical
{
						int vertIndex = vertInfos.size();
						vertInfos.push_back(VertexInfo(
...
//						tetraInfos[i][j][k][5].addVertexInfo(1, direction, vertIndex);
						tetraInfos[Index(i, j, k, 2)].addVertexInfo(2, direction, vertIndex);
						tetraInfos[Index(i, j, k, 5)].addVertexInfo(1, direction, vertIndex);

						if(i > 0){
//							tetraInfos[i-1][j][k][1].addVertexInfo(5, direction, vertIndex);
							tetraInfos[Index(i-1, j, k, 1)].addVertexInfo(5, direction, vertIndex);
						}
						if(j > 0){
//							tetraInfos[i][j-1][k][4].addVertexInfo(5, -direction, vertIndex);
							tetraInfos[Index(i, j-1, k, 4)].addVertexInfo(5, -direction, vertIndex);
						}
						if(i > 0 && j > 0){
//							tetraInfos[i-1][j-1][k][0].addVertexInfo(3, -direction, vertIndex);
//							tetraInfos[i-1][j-1][k][3].addVertexInfo(4, -direction, vertIndex);
							tetraInfos[Index(i-1, j-1, k, 0)].addVertexInfo(3, -direction, vertIndex);
							tetraInfos[Index(i-1, j-1, k, 3)].addVertexInfo(4, -direction, vertIndex);
						}
}
					}
				}
...
//					float intpPos = linearInterpolation(leftV, rightV, contour);
					...
					if(intpPos >= 0.0f && intpPos <= 1.0f){
						int latticeP = 3;
						const Normal n = linearInterpolation(normalField[i][j][k], normalField[i+1][j+1][k], normalField[i+1][j][k], normalField[i][j+1][k], intpPos);
//#pragma omp critical
{
						int vertIndex = vertInfos.size();
						vertInfos.push_back(VertexInfo(
...
//						tetraInfos[i][j][k][3].addVertexInfo(2, direction, vertIndex);
						tetraInfos[Index(i, j, k, 0)].addVertexInfo(1, direction, vertIndex);
						tetraInfos[Index(i, j, k, 3)].addVertexInfo(2, direction, vertIndex);

						if(k > 0){
//							tetraInfos[i][j][k-1][2].addVertexInfo(4, -direction, vertIndex);
//							tetraInfos[i][j][k-1][5].addVertexInfo(3, -direction, vertIndex);
							tetraInfos[Index(i, j, k-1, 2)].addVertexInfo(4, -direction, vertIndex);
							tetraInfos[Index(i, j, k-1, 5)].addVertexInfo(3, -direction, vertIndex);
						}
}
					}
				}
...
//					float intpPos = linearInterpolation(leftV, rightV, contour);
					...
					if(intpPos >= 0.0f && intpPos <= 1.0f){
						int latticeP = 6;
						const Normal n = linearInterpolation(normalField[i][j][k], normalField[i][j+1][k+1], normalField[i][j+1][k], normalField[i][j][k+1], intpPos);
//#pragma omp critical
{
						int vertIndex = vertInfos.size();
						vertInfos.push_back(VertexInfo(
...
//						tetraInfos[i][j][k][5].addVertexInfo(2, direction, vertIndex);
						tetraInfos[Index(i, j, k, 4)].addVertexInfo(1, direction, vertIndex);
						tetraInfos[Index(i, j, k, 5)].addVertexInfo(2, direction, vertIndex);

						if(i > 0){
//							tetraInfos[i-1][j][k][0].addVertexInfo(4, -direction, vertIndex);
//							tetraInfos[i-1][j][k][1].addVertexInfo(3, -direction, vertIndex);
							tetraInfos[Index(i-1, j, k, 0)].addVertexInfo(4, -direction, vertIndex);
							tetraInfos[Index(i-1, j, k, 1)].addVertexInfo(3, -direction, vertIndex);
						}
}
					}
				}
...
//					float intpPos = linearInterpolation(leftV, rightV, contour);
					...
					if(intpPos >= 0.0f && intpPos <= 1.0f){
						int latticeP = 5;
						const Normal n = linearInterpolation(normalField[i][j][k], normalField[i+1][j][k+1], normalField[i+1][j][k], normalField[i][j][k+1], intpPos);
//#pragma omp critical
{
						int vertIndex = vertInfos.size();
						vertInfos.push_back(VertexInfo(
...
//						tetraInfos[i][j][k][2].addVertexInfo(1, direction, vertIndex);
						tetraInfos[Index(i, j, k, 1)].addVertexInfo(2, direction, vertIndex);
						tetraInfos[Index(i, j, k, 2)].addVertexInfo(1, direction, vertIndex);

						if(j > 0){
//							tetraInfos[i][j-1][k][3].addVertexInfo(3, -direction, vertIndex);
//							tetraInfos[i][j-1][k][4].addVertexInfo(4, -direction, vertIndex);
							tetraInfos[Index(i, j-1, k, 3)].addVertexInfo(3, -direction, vertIndex);
							tetraInfos[Index(i, j-1, k, 4)].addVertexInfo(4, -direction, vertIndex);
						}
}
					}
				}
				...
				if(i < numX -1 && j < numY -1 && k < numZ -1){
					...
					if(intpPos >= 0.0f && intpPos <= 1.0f){
						int latticeP = 7;
						const Normal n = linearInterpolation(normalField[i][j][k], normalField[i+1][j+1][k+1], normalField[i+1][j][k], normalField[i][j+1][k], normalField[i][j][k+1], normalField[i+1][j+1][k], normalField[i][j+1][k+1], normalField[i+1][j][k+1], intpPos);
//#pragma omp critical
{
						int vertIndex = vertInfos.size();
						vertInfos.push_back(VertexInfo(
...
//						tetraInfos[i][j][k][5].addVertexInfo(0, direction, vertIndex);
						tetraInfos[Index(i, j, k, 0)].addVertexInfo(0, direction, vertIndex);
						tetraInfos[Index(i, j, k, 1)].addVertexInfo(0, direction, vertIndex);
						tetraInfos[Index(i, j, k, 2)].addVertexInfo(0, direction, vertIndex);
						tetraInfos[Index(i, j, k, 3)].addVertexInfo(0, direction, vertIndex);
						tetraInfos[Index(i, j, k, 4)].addVertexInfo(0, direction, vertIndex);
						tetraInfos[Index(i, j, k, 5)].addVertexInfo(0, direction, vertIndex);
}
					}
				}
			}
...
//		delete vertInfos[i];
	...
	for(map_index_tetra::iterator it = tetraInfos.begin(); it != tetraInfos.end(); ++it){
