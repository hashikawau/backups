    ...
    */
    ...
    void getAction(const int source, const String& text)
    {
        if(source == static_cast<int>(&_ttf[0]) ||
            source == static_cast<int>(&_ttf[1]))
        {
            FileDialog fd(*this, L"Load File");
            fd.setVisible(true);
            _ttf[0].setText(String(fd.getDirectory()) +String(fd.getFile()));
            //_ttf[0].setText(String(fd.getDirectory()));
            //_ttf[1].setText(String(fd.getFile()));
            
            //_ttf[2].setText(_ttf[0].getText() +_ttf[1].getText());
        }
        //if(source == static_cast<int>(&_ttf[1]))
        //    { _ttf[1].setText(String(_file->isFile())); }
...
int WINAPI WinMain(HINSTANCE hInstance,
                   ...
                   int nCmdShow)
{
    TestFrame test(String(L"TestFile.exe"));
