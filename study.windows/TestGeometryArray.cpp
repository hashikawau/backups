...
#include "windows\cpp\media\3d\LineArray.h"
...
Test::Test(const String& str): Frame(str)
{
    ...
    _button = new Button(String("Button"));
    //_button->setBounds();
...
{
    Button* b = reinterpret_cast<Button*>(e.getSource());
    Test& t = reinterpret_cast<Test&>((*b).getParent());
    //cout << &b << "  ";
    //cout << &t << endl;
    //t.repaint();
    
    //Graphics& g = t.getGraphics();
    Graphics& g = const_cast<Graphics&>(*_graphicsWork);
    g.setColor(Color::white);
    g.fillRect(0, 0, getWidth(), getHeight());
    g.setColor(Color::blue);
    //cout << (int)_graphicsWork << "  ";
    //cout << (int)&g << "  ";
    //cout << (int)(&(*_imageWork)) << endl;
    
    int xp[] = {50, 100, 120, 220};
    int yp[] = {50, 30, 120, 100};
    int np =4;
    int ncount =10000;
    int width =1;
    int eltime = timeGetTime();
    for(int i=0; i< ncount; ++i){
        g.drawLine(0, 0, 600, 600);
        //g.fillRect(i +width, 50, width, 40);
        //g.fillPolygon(xp, yp, np);
        //xp[3] += i;
        //g.setColor(Color(i));
    }
    cout << timeGetTime() -eltime << " , ";
    //g.setColor(Color::blue);
    eltime = timeGetTime();
    for(int i=0; i< ncount; ++i){
        g.drawPixel(0, 0);
        //g.drawPixel(0, 0);
        //g.fillRect2(i +width, 100, width, 40);
        //g.fillPolygon(xp, yp, np);
        //xp[3] += i;
        //g.setColor(Color(i));
    }
    cout << timeGetTime() -eltime << endl;
    
    Graphics& gw = t.getGraphics();
    gw.drawImage(*_imageWork, 0, 0, *this);
