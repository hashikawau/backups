	 ...
	 m_mvpMatrix(shared_ptr<GLSLUniformMatrix4>(new GLSLUniformMatrix4(GLMatrix4::createIdentity())))
{
	...
	m_vertex = shared_ptr<GLSLAttribPointer>(new GLSLAttribPointer(m_idVBOAttrib, 0, 0));
	m_glslProgram->setAttribute("a_vertex", m_vertex);

	glGenBuffers(1, &m_idVBOIndices);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_idVBOIndices);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_modelObject->getNumIndices() * sizeof(unsigned int), m_modelObject->getIndices().get(), GL_STATIC_DRAW);
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);

	m_glslProgram->setIndexVBO(m_idVBOIndices);
//	m_glslProgram->setPrimitive(GL_LINE_STRIP);
	m_glslProgram->setPrimitive(GL_TRIANGLES);
	m_glslProgram->setNumIndex(m_modelObject->getNumIndices());
