...
IllustrationLogicCanvas::IllustrationLogicCanvas()
{
    ...
    setNumGrid(String("iraroji/iraroji(15_15)-01.txt"));
...
void IllustrationLogicCanvas::calculateLine2(int indexline)
{
    ...
    for(int i=0; i< numblack; ++i){
        ...
        if(!doexist){
            ...
            int blackheadLast = numline -blacks[i];
            for(int j=0; j< i; ++j){
                blackheadFirst += blacks[j] +1;
            }
            for(int j= i+1; j< numblack; ++j){
                blackheadLast -= blacks[j] +1;
            }
            
            int j =0;
            for(; j< blackheadFirst; ++j){
                blackhead[i][j] =false;
            }
            for(; j<= blackheadLast; ++j){
                blackhead[i][j] =true;
            }
            for(; j< numline; ++j){
                blackhead[i][j] =false;
...
//{//black string
    ...
    for(int k=0; k< numstring; ++k){
        ...
        if(countpossible == 1){
            ...
            for(int j=0;
                j< chamberhead[countchamber];
                ++j)
            {
                blackhead[blackpossible][j] =false;
            }
            for(int j= chamberhead[countchamber];
                j< chamberhead[countchamber] +sizechamber[countchamber];
                ++j)
            {
                if(j > stringhead[k]){
                    blackhead[blackpossible][j] =false;
                }
                if(j +blacks[blackpossible]
                    < stringhead[k] +sizestring[k])
                {
                    blackhead[blackpossible][j] =false;
                }
                if(j +blacks[blackpossible]
                    > chamberhead[countchamber] +sizechamber[countchamber])
                {
                    blackhead[blackpossible][j] =false;
                }
            }
            for(int j= chamberhead[countchamber] +sizechamber[countchamber];
                j< numline;
                ++j)
            {
                blackhead[blackpossible][j] =false;
